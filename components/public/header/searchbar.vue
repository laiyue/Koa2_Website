      ...
      <el-col
        ...
        <p class="suggest">
          <nuxt-link
            v-for="(item,idx) in hotPlace"
            :key="idx"
            :to="'/products?keyword='+encodeURIComponent(item.name)">{{ item.name }}</nuxt-link>
        </p>
        <ul class="nav">
          <li><nuxt-link
            to="/"
...
export default {
  data(){
    return {
      search:'',
      isFocus:false,
      hotPlace:[{name:"火锅"},{name:"火锅"},{name:"火锅"},{name:"火锅"}],
      searchList:[{name:"车厘子"},{name:"车厘子"},{name:"车厘子"},{name:"车厘子"}]
    }
  },
  computed:{
    isHotPlace:function(){
